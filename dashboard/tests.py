...
class DashboardViewTests(TestCase):
    """
    Test suite for the dashboard view, which displays sprites and handles
    payment statuses.
    """
    def setUp(self):
        """
        Set up test environment by creating a user, a shelter, an animal, and a
        sprite associated with the user.
        """
    ...
    def test_dashboard_view_get(self):
        ...
        Confirms that the correct template is used and that the user's sprites
        and profile are in the context.
        """
    ...
    def test_dashboard_view_with_success_payment_status(self):
        """
        Test accessing the dashboard with a successful payment status. Confirms
        that a success message is displayed when payment is successful.
        """
        ...
        self.assertEqual(
            messages[0].message,
            "Payment successful! Received 100 tokens."
            )
    ...
    def test_dashboard_view_with_cancel_payment_status(self):
        ...
        self.assertEqual(
            messages[0].message,
            "Payment cancelled. No tokens added."
            )
        self.assertEqual(messages[0].tags, "error")

    def tearDown(self):
        """
        Clean up test environment by removing the uploaded image file after
        tests are done.
        """
...
class SelectSpriteViewTests(TestCase):
    """
    Test suite for the select sprite view, where users can foster an animal by
    selecting a sprite.
    """
    def setUp(self):
        """
        Set up test environment by creating a user, shelter, animal, and
        logging in the user.
        """
        self.user = User.objects.create_user(
            username='testuser',
    ...
    def test_select_sprite_get(self):
        ...
        response = self.client.get(
            f'/dashboard/select-sprite/{self.animal.id}/'
            )
        self.assertEqual(response.status_code, 200)
    ...
    def test_select_sprite_post_valid(self):
        """
        Test a valid POST request to ensure the sprite is created, the animal
        is fostered, and the user is redirected to the dashboard.
        """
    ...
    def test_select_sprite_post_invalid(self):
        """
        Test an invalid POST request to ensure the sprite is not created and
    ...
    def test_redirect_if_user_is_already_fosterer(self):
        """
        Test that a user who is already the fosterer is redirected and cannot
        foster the same animal again.
        """
...
class DeleteSpriteTests(TestCase):
    ...
    def test_sprite_deletion(self):
        """
        Test that deleting a sprite correctly updates the animal's fosterer and
        adoption status.
        """
        self.assertEqual(Sprite.objects.count(), 1)
    ...
    def test_unauthorized_sprite_deletion(self):
        ...
        self.assertEqual(
            str(messages[0]),
            "Not authorized to delete this sprite"
            )
...
class UpdateStatusViewTests(TestCase):
    ...
    def setUp(self):
        """
        Set up the test environment by creating a user, shelter, animal, and
        sprite, and logging in the user.
        """
        self.user = User.objects.create_user(
            username='testuser',
            password='12345'
    ...
    def test_update_status_view(self, mock_update_status):
        """
        Test the update status view, ensuring that it updates the sprite's
        status and returns the expected JSON response.
        """
...
class FeedSpriteTestCase(TestCase):
    """
    Test suite for the 'feed sprite' functionality, which increases a sprite's
    satiation and deducts tokens from the user.
    """
    def setUp(self):
        """
        Set up the test environment by creating a user, shelter, animal, and
        sprite, and logging in the user. Also, initialize tokens for the user
        profile.
        """
        self.user = User.objects.create_user(
            username='testuser',
    ...
    def test_feed_sprite_view(self):
        """
        Test that feeding the sprite increases its satiation, deducts a token
        from the user's profile, and returns a successful JSON response.
        """
...
class SpriteModelTests(TestCase):
    """
    Test suite for the Sprite model, including creation, updating status, and
    defaults.
    """
    def setUp(self):
        """
        Set up the test environment by creating a user, shelter, and animal for
        use in the tests.
        """
        self.user = User.objects.create_user(
            username='testuser',
    ...
    def test_sprite_defaults(self):
        """
        Test the default values for a sprite's breed, colour, and current
        state.
        """
        sprite = Sprite.objects.create(
            user=self.user,
    ...
    def test_string_representation(self):
        ...
        self.assertEqual(
            str(sprite),
            f"Sprite {sprite.id} for {sprite.animal.name}"
            )

    def test_update_status(self):
        """
        Test the update_status method to ensure it adjusts satiation and state
        ...
        Sprite.objects.filter(id=sprite.id).update(
            last_checked=timezone.now() - timezone.timedelta(minutes=10)
        )
    ...
    def test_update_status_no_negative_satiation(self):
        """
        Test that the sprite's satiation does not go below 0 after updating
        status.
        """
        sprite = Sprite.objects.create(
            user=self.user,
    ...
    def test_update_status_updates_last_checked(self):
        """
        Test that the sprite's last_checked field is correctly updated after
        ...
        sprite.update_status()
        self.assertEqual(
            sprite.last_checked.replace(microsecond=0),
            now.replace(microsecond=0)
            )

    def test_update_status_resets_state_timers(self):
        """
        Test that time_standing and time_running are reset when a new day
        starts.
        """
        now = timezone.now()
        ...
        Sprite.objects.filter(id=sprite.id).update(
            last_checked=timezone.now() - timezone.timedelta(days=1)
        )
        sprite.refresh_from_db()
